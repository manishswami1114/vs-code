    ...
    Modernizr.on('webp', function(result) {
        ...
        if (result) {
            $elem.find('image').each(function() {
                ...
                if ($dataSrc.includes('spider')) {
                    return
                }
                replaceStr = $dataSrc.indexOf('.png') !== -1 ? 'png' : 'jpg'
                $this.attr('xlink:href', str + $this.attr('data-src').replace(replaceStr, 'webp'));
            });
            $elem.find('img').each(function() {
                ...
                replaceStr = $dataSrc.indexOf('.png') !== -1 ? 'png' : 'jpg';
                $this.attr('src', str + $this.attr('data-src').replace(replaceStr, 'webp'));
            });
        } else {
            $elem.find('image').each(function() {
                var $this = $(this),
...
(function(w) {
    ...
    if ($('html').hasClass("index")) {
        ...
        function generateSVGAndAppend(svgID, svgClass, parentID) {
            ...
            svgElem.setAttribute("class", svgClass);
        ...
        w.$ani.attachHomeMouseEvents = function() {
            if (!w.isMobile.tablet) {
                ...
                for (var i = 0; i < svgPolys.length; i++) {
                    svgPolys[i].style[Modernizr.prefixed('transform')] = 'translateZ(' + (Math.floor(Math.random() * goldenNo) + 1) + 'px)';
                }
                document.getElementById("homeTextWrap").style[Modernizr.prefixed('transform')] = 'translateZ(1000px)';
        ...
        yellowBackPoints = [
            ...
            yellowBackPolys = [],
            PSLogoPoints = [
                [505.9, 298, 426, 344.1, 426, 251.8],
                [644.6, 250.9, 564.6, 297.1, 564.6, 204.8],
                [559, 397.3, 638.9, 443.5, 638.9, 351.1],
                [734.3, 198.9, 654.3, 245, 654.4, 152.7],
                [728.1, 295.1, 648.1, 341.1, 648.2, 248.9],
                [725.1, 398.9, 645.2, 445, 645.2, 352.7],
                [648.2, 346.5, 728.1, 392.6, 728.1, 300.3],
                [426.2, 248.8, 506.2, 295, 506.2, 202.6],
                [339.8, 203, 419.8, 249.2, 419.7, 156.9],
                [564.6, 300.3, 644.6, 346.5, 644.5, 254.2],
                [568.2, 199, 648.2, 245.2, 648.2, 152.9],
                [505.9, 202.7, 426, 248.8, 426, 156.5],
                [419.8, 251, 339.9, 297.1, 339.9, 204.8],
                [419.7, 343.3, 339.9, 389.4, 339.9, 297.1],
                [419.7, 248.6, 419.7, 340.9, 339.9, 294.8],
                [419.7, 346.5, 419.7, 438.8, 339.9, 392.7]
            ];

        function generateDesktopHomeSVGs() {
            for (var a = 0; a < polysInSVG; a++) {
                generateSVGAndAppend("purpleBack" + a, "svgPolys home-purple-back", "homeSVG");
                generateSVGAndAppend("purpleFront" + a, "svgPolys home-purple", "homeSVG");
                generateSVGAndAppend("greenBack" + a, "svgPolys home-green-back", "homeSVG");
                generateSVGAndAppend("greenFront" + a, "svgPolys home-green", "homeSVG");
                generateSVGAndAppend("yellowBack" + a, "svgPolys home-yellow-back", "homeSVG");
                generateSVGAndAppend("yellowFront" + a, "svgPolys home-yellow", "homeSVG");
            }
            for (var a = 0; a < polysInSVG; a++) {
                var s1 = Snap("#purpleFront" + [a]);
                s1.attr(svgParams);
                purpleFrontPolys[a] = s1.polygon(PSLogoPoints[a]);
                var s2 = Snap("#purpleBack" + [a]);
                s2.attr(svgParams);
                purpleBackPolys[a] = s2.polygon(PSLogoPoints[15 - a]);
                var s3 = Snap("#greenFront" + [a]);
                s3.attr(svgParams);
                greenFrontPolys[a] = s3.polygon(PSLogoPoints[a]);
                var s4 = Snap("#greenBack" + [a]);
                s4.attr(svgParams);
                greenBackPolys[a] = s4.polygon(PSLogoPoints[15 - a]);
                var s5 = Snap("#yellowFront" + [a]);
                s5.attr(svgParams);
                yellowFrontPolys[a] = s5.polygon(PSLogoPoints[a]);
                var s6 = Snap("#yellowBack" + [a]);
                s6.attr(svgParams);
                yellowBackPolys[a] = s6.polygon(PSLogoPoints[15 - a]);
            }
        }

        function generateMobileHomeSVGs(isTablet) {
            function addmobileSVG(polySVG, polyArray, Polys, svgClass, parent, isTablet, num) {
            ...
            addmobileSVG("purpleFront", purpleFrontPts, purpleFrontPolys, "svgPolys home-purple", "homeSVG", isTablet, 0);
            addmobileSVG("purpleBack", purpleBackPts, purpleBackPolys, "svgPolys home-purple-back", "homeSVG", isTablet, 1);
            addmobileSVG("greenFront", greenFrontPts, greenFrontPolys, "svgPolys home-green", "homeSVG", isTablet, 2);
            addmobileSVG("greenBack", greenBackPts, greenBackPolys, "svgPolys home-green-back", "homeSVG", isTablet, 3);
            addmobileSVG("yellowFront", yellowFrontPts, yellowFrontPolys, "svgPolys home-yellow", "homeSVG", isTablet, 4);
            addmobileSVG("yellowBack", yellowBackPts, yellowBackPolys, "svgPolys home-yellow-back", "homeSVG", isTablet, 5);
        }
        ...
        function convertToPS() {
            for (var b = 0; b < polysInSVG; b++) {
                purpleFrontPolys[b].polyAnimate(PSLogoPoints[b], 3000, mina.easeOutCubic, function() {})
                purpleBackPolys[b].polyAnimate(PSLogoPoints[15 - b], 3000, mina.easeOutCubic, function() {})
                greenFrontPolys[b].polyAnimate(PSLogoPoints[b], 3000, mina.easeOutCubic, function() {})
                greenBackPolys[b].polyAnimate(PSLogoPoints[15 - b], 3000, mina.easeOutCubic, function() {})
                yellowFrontPolys[b].polyAnimate(PSLogoPoints[b], 3000, mina.easeOutCubic, function() {})
                yellowBackPolys[b].polyAnimate(PSLogoPoints[15 - b], 3000, mina.easeOutCubic, function() {})
            }
        }

        function convertPSToIcosa() {
            for (var b = 0; b < polysInSVG; b++) {
                purpleFrontPolys[b].polyAnimate(purpleFrontPoints[b], 750, mina.easeOutCubic, function() {})
                purpleBackPolys[b].polyAnimate(purpleBackPoints[b], 750, mina.easeOutCubic, function() {})
                greenFrontPolys[b].polyAnimate(greenFrontPoints[b], 750, mina.easeOutCubic, function() {})
                greenBackPolys[b].polyAnimate(greenBackPoints[b], 750, mina.easeOutCubic, function() {})
                yellowFrontPolys[b].polyAnimate(yellowFrontPoints[b], 750, mina.easeOutCubic, function() {})
                yellowBackPolys[b].polyAnimate(yellowBackPoints[b], 750, mina.easeOutCubic, function() {})
            }
        }

        function defineAnimThings() {
            new TimelineMax().fromTo($hello, 0.8, {
                autoAlpha: 0,
                x: "80%"
            }, {
                autoAlpha: 1,
            ...
            }, 0).addCallback(convertPSToIcosa, 3, ["convertPSToIcosa"]).addCallback($ani.attachHomeMouseEvents, 4)
        }
        ...
        if (w.isMobile.any && w.$ani.winWidth < 600) {
            ...
            if (w.isMobile.tablet || window.retina || window.devicePixelRatio > 1.5) {
                generateMobileHomeSVGs("tablet");
            } else {
                preloaderFadeOutTransformOrigin = "top left"
                generateDesktopHomeSVGs()
            }
        }
        $ani.homeElemsAnim = function homeElemsAnim() {
            ...
            TweenMax.to($introAnim, 1, {
                ...
                onComplete: function() {
                    ...
                    TweenMax.fromTo("#mainLogo + sup", 1, {
                        autoAlpha: 0
                    }, {
                        ease: Back.easeOut,
                        scale: 1,
        ...
        function resetAllVals() {
            $("#name").val("").next(".styledLabel").removeAttr("aria-invalid").html('').attr("data-text", "name");
            $("#email").val("").next(".styledLabel").removeAttr("aria-invalid").html('').attr("data-text", "email");
            $("#message").val("").next(".styledLabel").removeAttr("aria-invalid").html('').attr("data-text", "message");
        ...
        form.addEventListener("submit", function(e) {
            ...
            ajax('POST', 'https://formspree.io/mpzlrzdo', formData, function(data) {
                console.log(data);
                ...
                $('#messageStatus').css("color", "#c22085").html("Oops!The mail service seems to be down!<br> you can resubmit or reach me on <a href='mailto:contact@prashantsani.com'>mailto:contact@prashantsani.com</a>").attr({
                    'role': 'alert'
                });
            });
            return false;
        ...
        window.$ani.setSMAnim = function setSMAnim(tweenArray, triggerElement, triggerHook, dur, funcOnStart) {
            ...
            new ScrollMagic.Scene({
                ...
                duration: dur
            }).setTween(tweenQuote).addTo(controller).on('start', funcOnStart)
        }

        function quoteAnimations(no) {
            var quoteIcosaArray = [];
            if (no % 2 === 0) {
                quoteIcosaArray.push(TweenMax.from("#quote" + no + "Top", 0.5, {
                    x: "100%",
                    y: "-10%"
                }))
                quoteIcosaArray.push(TweenMax.from("#quote" + no + "Bottom", 0.5, {
                    x: "-100%",
                    y: "10%"
                }))
            } else {
            ...
            switch (no) {
                case 1:
                    w.$ani.setSMAnim([TweenMax.from(".quote1Heart", 0.9, {
                        ...
                        x: '30',
                        ease: Power1.easeIn
                    })], '.quote1TextBottom', "onEnter", '60%');
                    break;
                case 2:
                    w.$ani.setSMAnim([TweenMax.from("#quotePositiveTop", 2, {
                        strokeDashoffset: '1000',
                        ease: Power2.easeIn
                        ...
                        x: 70
                    })], '#quotePositiveBottom', "onEnter", '60%');
                    break;
                case 3:
                    w.$ani.setSMAnim([TweenMax.from(".quote3Poem", 1, {
                        ...
                        ease: Power2.easeIn
                    })], '#quote3 .quoteTextBG', "onEnter", '60%');
                    break;
                case 4:
                    w.$ani.setSMAnim([TweenMax.from("#quote4Triangle", 1.5, {
                        delay: 0.3,
                        strokeDashoffset: '1000',
                        ease: Power1.easeIn
                    })], '#quote4TextTop', "onEnter", '60%');
                    break;
                default:
                    break;
            }
        ...
        if (w.$ani.winWidth < 601) {
            w.$ani.setSMAnim([TweenMax.staggerFrom(".expLinks", 0.5, {
                opacity: 0,
                ease: Expo.easeInOut,
                scale: 0,
                transformOrigin: "center"
            }, 0.1), TweenMax.staggerFrom(".expFrame", 0.5, {
                delay: 0.1,
                scale: 0,
                opacity: 0,
                ease: Expo.easeInOut,
            ...
            w.$ani.setSMAnim([TweenMax.staggerFromTo(".expFrame", 1, {
                x: "-10%",
                y: "-7%"
            }, {
                x: "0%",
            ...
            w.$ani.setSMAnim([TweenMax.fromTo(".expFrame", 1, {
                x: "0%",
                y: "0%"
            }, {
                ease: Power3.easeOut,
        ...
        $('#projectsList li').each(function(index) {
            var $this = $(this);
            w.$ani.setSMAnim([TweenMax.from($this, 0.4, {
                opacity: 0,
                ease: Power1.easeOut
            }), TweenMax.fromTo($(this).find('.reveler'), 1.8, {
                x: "100%"
            }, {
                x: "-100%",
                ease: Expo.easeOut
            }), TweenMax.fromTo($this.find('article'), 1.3, {
                y: "100%"
            }, {
                delay: 0.3,
                y: "0%",
        ...
        w.$ani.setSMAnim([TweenMax.from("#aboutWindow", 0.9, {
            scale: 0,
            ease: Expo.easeInOut,
        ...
        }), TweenMax.staggerFrom(".aboutThoughtCloud", 0.5, {
            opacity: 0,
            ease: Expo.easeOut,
            delay: 0.4,
            scale: 0.5
        ...
        if (w.isMobile.any) {
            ...
            w.$ani.setSMAnim([TweenMax.from(".trainingIcosa0", 1, {
                ease: Back.easeInOut,
                scale: 0
            }, 0), TweenMax.from(".trainingIcosa5", 0.8, {
                ease: Power4.easeInOut,
                delay: 0.3,
                scale: 0,
                y: "3%"
            }, "2.5"), TweenMax.from(".trainingIcosa1", 0.7, {
                ease: Power2.easeInOut,
                delay: 1.4,
                scale: 0,
                y: "-150%"
            }, "2.5"), TweenMax.from(".trainingIcosa3", 0.9, {
                ease: Power3.easeInOut,
                delay: 1.1,
                autoAlpha: 0,
                scale: 2,
            ...
            w.$ani.setSMAnim([TweenMax.from(".trainingIcosa0", 0.5, {
                scale: 0
...
window.$ani.initCSAnims = function() {
    ...
    function setSMAnim2(tweenArray, triggerElement, triggerHook, dur, funcOnStart, reverse) {
        var tweenQuote = new TimelineMax();
        tweenQuote.add(tweenArray);
        new ScrollMagic.Scene({
            triggerElement: triggerElement,
            ...
            duration: dur,
            reverse: reverse
        }).setTween(tweenQuote).addTo(controller2).on('start', funcOnStart)
    }
    $('.CSSBrowser').each(function(index) {
        ...
        setSMAnim2([TweenMax.fromTo($CSSBrowserRevealer, 4, {
            y: "0%"
        }, {
            y: "100%",
            ease: Power4.easeOut
        })], this, 0.7, "80%", undefined, true);
    });
    $('.CS-MacBookWrap').each(function(index) {
        ...
        if ($this.prev().hasClass('CS-MacBookWrap') && (index % 2 !== 0) && (index !== 0)) {
            obj = {
                x: "-90%",
                scale: 1.1,
                autoAlpha: 0,
    ...
    $('.CS-manyiPadsWrap,.CS-MobileDevicesWrap').each(function(index) {
        ...
        $t.find('.CS-iPad').each(function(index2) {
            ...
            setSMAnim2([TweenMax.from(this, 0.6, opt)], this, 0.99, 0, undefined, false);
        });
        ...
        setSMAnim2([TweenMax.from(this, 1, {
            autoAlpha: 0
        })], this, 0.9, 0, undefined, false);
    });
    $('.CSSBrowserWrap.CS-DarkShadowWrap').each(function(index) {
        var DarkShadow = $(this).find('.CS-DarkShadow');
        setSMAnim2([TweenMax.fromTo(DarkShadow, 10, {
            y: '40%'
        }, {
            y: '-150%',
            ease: Power3.easeInOut
        })], this, 0.95, "200%", undefined, true);
    });
    $('.CS-Cover').each(function(index) {
        $this = $(this);
        setSMAnim2([TweenMax.fromTo($this.find(".CS-Cover-front"), 1800, {
            y: '35%'
        }, {
            y: '0%',
            ease: Power2.easeInOut
        }), TweenMax.fromTo($this.find(".CS-Cover-blurred"), 1300, {
            y: '0%'
        }, {
            y: '-10%',
            ease: Power1.easeInOut
        })], this, 0.6, "85%", undefined, true);
    });
};
(function(w) {
    ...
    function transitionPage($elementFrom, $elementTo, animStartCallBack, animFinalCallBack, direction) {
        w.$ani.isNavBtnClickable = false;
        console.log('disabling clicks')
        var _animStartCallBack = animStartCallBack ? animStartCallBack : function() {},
            _animFinalCallBack;
        ...
        startTeleportAnim($elementFrom, $elementTo, _animStartCallBack, _animFinalCallBack, direction);
        if ($elementFrom === $projectDetailsModal) {}
    }

    function startTeleportAnim($elementTo, $elementFrom, animStartCallBack, animFinalCallBack, direction) {
        ...
        TweenMax.set([$teleportAnim, $elementTo, $first, $second, $third, $fourth], teleportAnim_Init);
        ...
        Anim.addCallback(animStartCallBack).to($elementFrom, 1.1, elementFrom_F).to($first, 0.9, teleportAnimDIV_F, "0").to($second, 0.7, teleportAnimDIV_F, "0.7").to($third, 0.6, teleportAnimDIV_F, "1.2").to($fourth, 0.5, teleportAnimDIV_F, "1.7").to($elementTo, 0.4, elementTo_F, "2").addCallback(animFinalCallBack, "1.9")
    }

    function staggerNavItems() {
        TweenMax.staggerFromTo("#nav li", 1.2, {
            scale: 0.8,
            cycle: {
                x: function(i) {
                    return -i * 20;
    ...
    $hamburger.click(function() {
        if (!w.$ani.isNavBtnClickable)
            return false
        if (!$hamburger.hasClass('clicked')) {
            changeModalAria($nav, $main);
            transitionPage($nav, $main, function() {
            ...
            if ($html.attr('data-section-active') === 'projectDetailsModal') {
                ...
                transitionPage($main, $projectDetailsModal, function() {
                    console.log('---------------');
                    console.log('closing projs');
                    $hamburger.blur();
                ...
                transitionPage($main, $nav, function() {
        ...
        popstate: function(stateObj) {
            ...
            if (stateObj.type === "projects") {
                if ($html.attr('data-section-active') === "main") {
                    ...
                    $ani.loadProj(stateObj.page, stateObj.title, stateObj.type, "noupdate");
                    $("html").css({
                        'overflow': 'hidden'
                    });
                }
    ...
    w.$ani.loadProj = function(href, title, pathPrefix, direction) {
        console.log('Placing a Ajax request for ' + href)
        if ($html.attr('data-section-active') === "projectDetailsModal") {
    ...
    $(document).on('click', '#projectsList .openPortfolio', function(event) {
        event.preventDefault();
        var $this = $(this);
        ...
        $projectDetailsModal.html('');
        transitionPage($projectDetailsModal, $main, function() {
            console.log('opening up :' + href)
        }, function() {
            console.log('---------------');
        });
        w.$ani.loadProj(href, title, "projects/", "Click from Proj List : Home");
        changeModalAria($projectDetailsModal, $main);
        return false
    });
    $(document).on('click', '#CS-projectsList .openPortfolio', function(event) {
        ...
        TweenMax.to($projectDetailsModalContent, 0.3, {
            ...
            onComplete: function() {
                ...
                w.$ani.loadProj(href, title, "", "Click from link inside Project Modal");
            }
        });
        return false
    });
    ...
    function changeCSS(w) {
        ...
        if (w.winHeight < 350) {
            navTop = "100px",
                navLeft = parseInt(w.winWidth / 20) + "px"
        }
        $('#nav ul').css({
            top: navTop,
            left: navLeft
